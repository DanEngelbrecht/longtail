...
static int SafeCreateDirectory(StorageAPI* storage_api, const char* path)
{
...
static int EnsureParentPathExists(StorageAPI* storage_api, const char* path)
{
...
VersionIndex* CreateVersionIndex(StorageAPI* storage_api, HashAPI* hash_api, Bikeshed shed, const char* root_path, const Paths* paths)
{
    ...
    {
        LOG("Failed to hash assets in `%s`\n", root_path);
        free(contentHashes);
        free(pathHashes);
        free(contentSizes);
        return 0;
...
struct TroveStorageAPI
{
    ...
    static int CreateDirectory(StorageAPI* , const char* path)
    {
        char* tmp_path = strdup(path);
        Trove_DenormalizePath(tmp_path);
...
void Bench()
{
    ...
    for (uint32_t i = 0; i < VERSION_COUNT; ++i)
    {
        ...
        VersionIndex* version_index = CreateVersionIndex(&storage_api.m_StorageAPI, &hash_api.m_HashAPI, shed, version_source_folder, version_source_paths);
    ...
    LONGTAIL_ATOMICADD_PRIVATE(&stop, 1);
...
//    #define VERSION2_FOLDER "version2"
    ...
    VersionIndex* version1 = CreateVersionIndex(&storage_api.m_StorageAPI, &hash_api.m_HashAPI, shed, local_path_1, local_path_1_paths);
    ...
    ASSERT_EQ(1, ReconstructVersion(&storage_api.m_StorageAPI, &compression_api.m_CompressionAPI, shed, local_content_index, version1, local_content_path, remote_path_1));
    ...
    VersionIndex* version2 = CreateVersionIndex(&storage_api.m_StorageAPI, &hash_api.m_HashAPI, shed, local_path_2, local_path_2_paths);
...
//    }
    ...
    ASSERT_EQ(1, ReconstructVersion(&storage_api.m_StorageAPI, &compression_api.m_CompressionAPI, shed, merged_local_content, version2, local_content_path, remote_path_2));
    printf("Reconstructed %" PRIu64 " assets to `%s`\n", *version2->m_AssetCount, remote_path_2);
