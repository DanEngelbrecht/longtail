...
static void InMemStorageAPI_Dispose(struct Longtail_API* storage_api)
{
    ...
    Longtail_DeleteRWLock(in_mem_storage_api->m_Lock);
...
static int InMemStorageAPI_OpenReadFile(struct Longtail_StorageAPI* storage_api, const char* path, Longtail_StorageAPI_HOpenFile* out_open_file)
{
    ...
    Longtail_LockRWReadLock(instance->m_Lock);
    intptr_t tmp;
    intptr_t it = hmgeti_ts(instance->m_PathHashToContent, path_hash, tmp);
    if (it != -1)
    {
        ...
        {
            Longtail_UnlockRWReadLock(instance->m_Lock);
...
static int InMemStorageAPI_OpenWriteFile(struct Longtail_StorageAPI* storage_api, const char* path, uint64_t initial_size, Longtail_StorageAPI_HOpenFile* out_open_file)
{
    ...
    Longtail_LockRWWriteLock(instance->m_Lock);
    intptr_t tmp;
    if (parent_path_hash != 0 && hmgeti_ts(instance->m_PathHashToContent, parent_path_hash, tmp) == -1)
    {
        Longtail_UnlockRWWriteLock(instance->m_Lock);
...
static int InMemStorageAPI_CreateDir(struct Longtail_StorageAPI* storage_api, const char* path)
{
    ...
    intptr_t source_path_ptr = hmgeti_ts(instance->m_PathHashToContent, path_hash, tmp);
...
static int InMemStorageAPI_RenameFile(struct Longtail_StorageAPI* storage_api, const char* source_path, const char* target_path)
{
    ...
    Longtail_LockRWWriteLock(instance->m_Lock);
    intptr_t tmp;
    intptr_t source_path_ptr = hmgeti_ts(instance->m_PathHashToContent, source_path_hash, tmp);
    if (source_path_ptr == -1)
    {
        Longtail_UnlockRWWriteLock(instance->m_Lock);
    ...
    intptr_t target_path_ptr = hmgeti_ts(instance->m_PathHashToContent, target_path_hash, tmp);
...
static int InMemStorageAPI_Init(struct InMemStorageAPI* storage_api)
{
    ...
    storage_api->m_InMemStorageAPI.GetPermissions = InMemStorageAPI_GetPermissions;
    ...
    int err = Longtail_CreateRWLock(&storage_api[1], &storage_api->m_Lock);
    if (err)
    {
...
{
    struct InMemStorageAPI* storage_api = (struct InMemStorageAPI*)Longtail_Alloc(sizeof(struct InMemStorageAPI) + Longtail_GetRWLockSize());
    if (!storage_api)
    {
